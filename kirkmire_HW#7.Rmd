...
```
There is very little evidence against the null hypothesis that acceptance is independent of gender. There does not appear to be a strong relationship between sex and accept.  
...
The deviance decreases only slightly slightly provides little evidence against the null hypothesis that the added square terms are zero. Therefore, it is not neccesary or advisable to add the squared terms to the model.
...
The deviance decreases only slightly in the model that includes the squares of MCAT and GPA which provides little evidence against the null hypothesis that the added pairwise  are zero. Therefore, it is not neccesary or advisable to add the squared terms to the model.
...
MCAT: The odds of acceptance increases by between .98 and 1.51 times for every increase in one point of MCAT score given that all other variables are held constant. This interval contains one which indicates that MCAT score increases do not greatly increase odds of admission given that other variables are constant. 
