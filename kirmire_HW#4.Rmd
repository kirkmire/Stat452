...
p <- ggplot(Crabs, aes(logForce,logHeight))
p + geom_point(aes(colour = factor(Crabs$Species)),size = 3)+
...
```{r}
model1<-lm(Crabs$Height~Crabs$logForce+Crabs$Species+Crabs$Force:Crabs$Species)
...
• Garage and Price both have big P-values. Could we reasonably eliminate both? Test
