...
mew.B1<-(BAF*(sum(tf(B_1))))
mew.B2<-(BAF*(sum(tf(B_2))))
mew.B3<-(BAF*(sum(tf(B_3))))
mew.B4<-(BAF*(sum(tf(B_4))))
mew.B5<-(BAF*(sum(tf(B_5))))
    
est.tpHA_B<-((sum(mew.B1,mew.B2,mew.B3,mew.B4,mew.B5))/5)*.7
...
avediam <- function(x) {
   result<-x/(((((x)/100)/2)^2)*3.14)
   return(result)
}


ad.B1<-(BAF*(sum(avediam(B_1))))
ad.B2<-(BAF*(sum(avediam(B_2))))
ad.B3<-(BAF*(sum(avediam(B_3))))
ad.B4<-(BAF*(sum(avediam(B_4))))
ad.B5<-(BAF*(sum(avediam(B_5))))
