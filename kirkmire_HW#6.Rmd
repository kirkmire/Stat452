...
f.logt.sat.lm<-lm(sat$SAT~log(sat$Takers)+sat$Rank+sat$Expend+sat$Years)
...
sansAK.lm<-lm(sat.sansAK$SAT~log(sat.sansAK$Takers)+sat.sansAK$Rank+sat.sansAK$Expend+sat.sansAK$Years)

#resid vs fitted plot#
plot(resid(sansAK.lm)~sansAK.lm$fitted.values)

#Cooks Distance#
plot(sansAK.lm,which=c(4))

#leverage#
lev1<-hat(model.matrix(sansAK.lm))
